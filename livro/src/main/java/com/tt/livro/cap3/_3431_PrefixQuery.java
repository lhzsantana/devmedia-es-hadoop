...
public class _3431_PrefixQuery extends _0_Abstract {

	public _3431_PrefixQuery() {
		_211_Connection connection = new _211_Connection();

		client = connection.createClient();
	}

	public void searchBoolQuery(String prefixoTitulo) {
				
		SearchResponse response = client.prepareSearch(index).setTypes(type)
				.setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
				.setQuery(
						QueryBuilders.prefixQuery("titulo", prefixoTitulo)
				)
				.execute()
				.actionGet();
